...
$install = @('-s', $Serial, 'install', '-r')
if ($InstallAbi) { $install += @('--abi', $InstallAbi) }
& $Adb @install $app
if ($LASTEXITCODE -ne 0) { throw '宿主安装失败' }
& $Adb @install $test
if ($LASTEXITCODE -ne 0) { throw '测试 APK 安装失败' }
& $Adb -s $Serial shell getprop ro.build.fingerprint | Out-File $environmentLog
& $Adb -s $Serial shell getprop ro.product.cpu.abilist | Out-File $environmentLog -Append
& $Adb -s $Serial shell dumpsys package com.zhishurufa.sample |
    Select-String 'primaryCpuAbi|secondaryCpuAbi' | Out-File $environmentLog -Append

$command = @('-s', $Serial, 'shell', 'am', 'instrument', '-w', '-r')
...
if ($exitCode -ne 0 -or $output -notmatch 'OK \(9 tests\)' -or $output -match 'Process crashed|FAILURES!!!|INSTRUMENTATION_FAILED') {
    Get-Content -LiteralPath $log -Tail 50
    & $Adb -s $Serial logcat -d -b crash | Out-File "$log.crash.txt"
    throw "设备验收未通过，日志：$log"
}
Write-Output "通过：9 组设备测试（$Variant，$abiName）。日志：$log"
