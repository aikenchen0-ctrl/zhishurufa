...
Java_com_osfans_trime_core_Rime_getRimeCandidates(JNIEnv* env, jclass clazz,
                                                  jint start_index,
                                                  jint limit) {
  if (start_index < 0 || limit < 0 || limit > 1024) {
    auto exception = env->FindClass("java/lang/IllegalArgumentException");
    env->ThrowNew(exception, "Invalid candidate range");
    env->DeleteLocalRef(exception);
    return nullptr;
  }
  if (limit == 0) {
    return env->NewObjectArray(0, GlobalRef->CandidateProto, nullptr);
  }
