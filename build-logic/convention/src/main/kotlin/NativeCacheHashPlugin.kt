            ...
            with(project) {
                val magic =
                    buildString {
                        ...
                        fileTree(nativeSourceDir.resolve("cmake")).forEach { module ->
                            appendLine(sha256(module))
                        }
                        fileTree(nativeSourceDir.resolve("librime_jni")).forEach { source ->
                            appendLine(sha256(source))
                        }
                        appendLine(sha256(nativeSourceDir.resolve("CMakeLists.txt")))
                    }
