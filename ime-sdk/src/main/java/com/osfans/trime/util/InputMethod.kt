...
fun InputConnection.monitorCursorAnchor(enable: Boolean = true, immediate: Boolean = false): Boolean {
    if (!enable) {
        requestCursorUpdates(0)
        return false
    }
    var scheduled = false
    val mode = InputConnection.CURSOR_UPDATE_MONITOR or if (immediate) InputConnection.CURSOR_UPDATE_IMMEDIATE else 0
    ...
    if (!scheduled) {
        scheduled = requestCursorUpdates(mode)
    }
    return scheduled
}
