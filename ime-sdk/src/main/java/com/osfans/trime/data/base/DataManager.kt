...
object DataManager {
    ...
    private val dataDir: File =
        ...
        }).let { if (SdkStorage.usesLegacyPaths(appContext)) it else File(it, "trime-sdk").also(File::mkdirs) }
    ...
    val sharedDataDir = File(SdkStorage.externalRoot(appContext), "shared").also { it.mkdirs() }

    private val runtimeUserDataDir =
        File(SdkStorage.externalRoot(appContext), "rime").also { it.mkdirs() }
    ...
    fun sync() = lock.withLock {
        ...
        if (!custom.exists()) {
            val defaults = appContext.assets.open("shared/builtin.default.yaml").bufferedReader().use { it.readText() }
            custom.writeText(defaults)
        }
