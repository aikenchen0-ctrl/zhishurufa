...
class RimeDataProvider : DocumentsProvider() {
    ...
    private lateinit var paths: DocumentPathResolver
    private lateinit var textFilePaths: Array<String>

    private val File.docId
        get() = paths.id(this)

    private fun fileFromDocId(docId: String) = paths.resolve(docId)

    override fun onCreate(): Boolean {
        baseDir = SdkStorage.externalRoot(context ?: return false)
        paths = DocumentPathResolver(baseDir)
    ...
    override fun queryChildDocuments(
        ...
        fileFromDocId(parentDocumentId).listFiles()?.filter { paths.contains(it) }?.forEach {
            newRowFromFile(it)
        }
    }
    ...
    override fun isChildDocument(
        parentDocumentId: String,
        documentId: String,
    ): Boolean = paths.isChild(parentDocumentId, documentId)
    ...
    override fun copyDocument(
        ...
        if (oldFile.isDirectory && newFile.path.startsWith(oldFile.path + File.separator)) {
            throw FileNotFoundException("不能将目录复制到自身内部")
        }
        oldFile.apply {
            try {
                val ok =
    ...
    override fun renameDocument(
        ...
        paths.requireMutable(oldFile)
        val newFile = paths.child(oldFile.parentFile!!, displayName)
        if (newFile.exists()) {
            throw FileNotFoundException("renameDocument id=$documentId to $displayName failed: target exists")
        }
        if (!oldFile.renameTo(newFile)) throw FileNotFoundException("重命名失败")
        return newFile.docId
    }
    ...
    override fun moveDocument(
        ...
        paths.requireMutable(oldFile)
        val newFile = createAbstractFile(targetParentDocumentId, oldFile.name)
        if (!oldFile.renameTo(newFile)) throw FileNotFoundException("移动失败")
        return newFile.docId
    }
    ...
    override fun querySearchDocuments(
        ...
        fileFromDocId(rootId)
            .walk()
            .onEnter { paths.contains(it) }
            .filter { paths.contains(it) }
    ...
    private fun createAbstractFile(
        ...
        var newFile = paths.child(parent, displayName)
        var noConflictId = 2
        while (newFile.exists()) {
            newFile = paths.child(parent, "$displayName ($noConflictId)")
            noConflictId += 1
        }
        return newFile
    }
    ...
    private fun MatrixCursor.newRowFromFile(file: File) {
        ...
        if (file.canonicalFile != paths.root && file.parentFile?.canWrite() == true) {
            flags = flags or
