...
import com.osfans.trime.sdk.DraftKeyboardHeightPolicy
...
class InputView(
    ...
    private val draftPanel = DraftPanelView(context, TrimeSdk.drafts, DraftSurface.IME)
    private var baseKeyboardHeight = 0
    private var draftBodyHeight = 60
    private var draftNormalBodyHeight = 60
    ...
    init {
        draftPanel.onBodyHeightChanged = { bodyHeight, normalHeight ->
            draftBodyHeight = bodyHeight
            draftNormalBodyHeight = normalHeight
            updateKeyboardWindowHeight()
        }
        ...
        keyboardView =
            constraintLayout {
                ...
                add(
                    draftPanel,
                    lParams(matchParent, wrapContent) {
                        below(inputBar.contextActionUi)
                        centerHorizontally()
                    },
                )
                add(
    ...
    private fun refreshDraftColors() {
        draftPanel.refreshColors(scope.colors.candidateBackground, scope.colors.candidateTextColor)
    }

    private fun updateKeyboardWindowHeight() {
        if (baseKeyboardHeight <= 0) return
        val minimumHeight = dp(48)
        val maximumExtraHeight = dp(160)
        val height = DraftKeyboardHeightPolicy.resolve(
            baseHeight = baseKeyboardHeight,
            bodyHeight = draftBodyHeight,
            normalBodyHeight = draftNormalBodyHeight.coerceAtLeast(1),
            minimumHeight = minimumHeight,
            maximumExtraHeight = maximumExtraHeight,
        )
        windowManager.view.updateLayoutParams { this.height = height }
    }
    ...
    fun requestKeyboardLayout(layout: KeyboardLayout): Boolean = keyboardWindow.requestLayout(layout)

    fun currentKeyboardLayoutId(): String = keyboardWindow.currentLayoutId
