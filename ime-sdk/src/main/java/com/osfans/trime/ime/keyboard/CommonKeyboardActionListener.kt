...
import com.osfans.trime.sdk.AiDraftRequest
...
class CommonKeyboardActionListener(override val di: DI) : DIAware {
    ...
    /** 输入法内部上下文操作与工具栏共用同一份草稿生成链路。 */
    fun requestAiOperation(operation: AiDraftOperation) {
        handleAiDraft(operation.instruction)
    }

    private fun handleAiDraft(instruction: String) {
        service.lifecycleScope.launch {
            // 用户通过明确操作触发时，才打开当前编辑器上下文读取。
            TrimeSdk.setEditorContextEnabled(true)
            val snapshot = TrimeSdk.refreshEditorSnapshot()
            if (snapshot == null) {
                Toast.makeText(service, R.string.ai_draft_unavailable, Toast.LENGTH_SHORT).show()
                return@launch
            }
            val contextText = snapshot.selectedText ?: snapshot.text
            // 同一输入框上下文只保留一个 SDK 生成槽；宿主仍可通过 DraftController 推送多条独立草稿。
            val draftId = "ime-context"
            val draftTitle = service.getString(R.string.ai_draft_title)
            val request = AiDraftRequest(instruction.ifBlank { "根据当前内容生成自然、简洁的回复" }, contextText)
            val result = runCatching {
                TrimeSdk.ai.generateDraftStreaming(
                    request,
                    draftId,
                    draftTitle,
                    onChunk = { partial ->
                    if (partial.isNotBlank()) {
                        val preview = DraftItem(
                            draftId,
                            draftTitle,
                            listOf(DraftSegment(partial)),
                            request,
                        )
                        TrimeSdk.drafts.upsertAndBind(preview, snapshot)
                    }
                    },
                    requestKey = "ime-context",
                )
            }.getOrElse {
                Timber.w(it, "AI 草稿生成失败")
                null
            }
            if (result == null) {
                Toast.makeText(service, R.string.ai_draft_unavailable, Toast.LENGTH_SHORT).show()
            } else {
                TrimeSdk.drafts.upsertAndBind(result, snapshot)
                Toast.makeText(service, R.string.ai_draft_ready, Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    val listener by lazy {
        object : KeyboardActionListener {
            ...
            private fun handleFunctionCommand(action: KeyAction) {
                ...
                when (action.command) {
                    "host_action" -> TrimeSdk.dispatchHostAction(action.select, arg)
                    "ai_draft" -> handleAiDraft(arg)
                    "ai_menu" -> showAiMenu()
                    "draft_next" -> TrimeSdk.drafts.next()
                    "select_schema" -> service.postRimeJob {
                        commitComposition()
                        selectSchema(arg)
                    }
            ...
            private fun showAiMenu() {
                val operations = AiDraftOperation.entries.toTypedArray()
                val labels = operations.map {
                    when (it) {
                        AiDraftOperation.REPLY -> service.getString(R.string.ai_action_reply)
                        AiDraftOperation.REWRITE -> service.getString(R.string.ai_action_rewrite)
                        AiDraftOperation.POLISH -> service.getString(R.string.ai_action_polish)
                        AiDraftOperation.CONTINUE -> service.getString(R.string.ai_action_continue)
                        AiDraftOperation.SUGGEST -> service.getString(R.string.ai_action_suggest)
                    }
                }.toTypedArray()
                service.showDialog(
                    AlertDialog.Builder(service, R.style.Theme_TrimeSdkDialogTheme)
                        .setTitle(R.string.ai_action_menu_title)
                        .setItems(labels) { _, which ->
                            if (operations[which] == AiDraftOperation.SUGGEST) {
                                handleAiSuggestions()
                            } else {
                                handleAiDraft(operations[which].instruction)
                            }
                        }
            ...
            private fun handleAiSuggestions() {
                service.lifecycleScope.launch {
                    // 用户明确选择 AI 操作后才读取编辑器内容。
                    TrimeSdk.setEditorContextEnabled(true)
                    val snapshot = TrimeSdk.refreshEditorSnapshot()
                    if (snapshot == null) {
                        Toast.makeText(service, R.string.ai_draft_unavailable, Toast.LENGTH_SHORT).show()
                    ...
                    val request = AiDraftRequest(
                        AiDraftOperation.SUGGEST.instruction,
                        contextText,
                        maxOutputTokens = 96,
                    )
                    val result = runCatching {
                        TrimeSdk.ai.generateAndPublishSuggestions(
                            request,
                            TrimeSdk.suggestions,
                            "ai-suggestion",
                        )
                    }.getOrElse {
                        Timber.w(it, "AI 短推荐生成失败")
                        null
                    }
                    Toast.makeText(
                        service,
                        if (result.isNullOrEmpty()) R.string.ai_draft_unavailable else R.string.ai_suggestion_ready,
                        Toast.LENGTH_SHORT,
                    ).show()
                }
            }
