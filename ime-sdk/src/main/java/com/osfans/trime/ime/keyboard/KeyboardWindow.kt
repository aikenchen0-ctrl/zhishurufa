...
class KeyboardWindow(di: DI) :
    ...
    private fun attachKeyboard(target: String, synchronizeMode: Boolean = true) {
        ...
        keyboard.also {
            ...
            currentKeyboard = it
            TrimeSdk.publishKeyboardLayout(target)
        }
    ...
    /** 由 SDK 控制器调用；未知或当前主题缺少布局时拒绝请求。 */
    fun requestLayout(layout: KeyboardLayout): Boolean {
        if (layout.id !in presetKeyboardIds) return false
        switchKeyboard(layout.id)
        return true
    }
