...
open class TrimeInputMethodService : LifecycleInputMethodService() {
    ...
    private var lastEditorCursor: EditorCursorAnchor? = null
    ...
    override fun onUpdateCursorAnchorInfo(info: CursorAnchorInfo) {
        val editorBounds = info.getCharacterBounds(0)?.let { RectF(it) } ?: RectF(
            info.insertionMarkerHorizontal,
            info.insertionMarkerTop,
            info.insertionMarkerHorizontal,
            info.insertionMarkerBottom,
        )
        info.matrix.mapRect(editorBounds)
        lastEditorCursor = EditorCursorAnchor(
            left = editorBounds.left,
            top = editorBounds.top,
            right = editorBounds.right,
            bottom = editorBounds.bottom,
            valid = !editorBounds.any(Float::isNaN),
        )
    ...
    override fun onUpdateSelection(
        ...
        publishEditorSnapshot(newSelStart, newSelEnd)
    }
    ...
    private fun publishEditorSnapshot(
        ...
        TrimeSdk.publishEditorSnapshot(
            EditorSnapshot(
                text = text,
                selectionStart = selectionStart ?: extracted.selectionStart,
                selectionEnd = selectionEnd ?: extracted.selectionEnd,
                cursor = lastEditorCursor,
            ),
        )
    }
    ...
    override fun onCreateInputView(): View? {
        ...
        replaceInputViews(themeScope)
        // 返回真实视图，兼容不接受“先 setInputView 后返回 null”的定制系统。
        return inputView
    }
