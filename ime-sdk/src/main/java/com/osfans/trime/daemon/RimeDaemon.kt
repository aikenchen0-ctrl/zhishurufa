...
object RimeDaemon {
    ...
    private fun establish(name: String) = object : RimeSession, RimeSessionBridge {
        ...
        override suspend fun <T> runOnReadyWithBoundary(jobId: Long, block: suspend RimeApi.() -> T): T = ensureEstablished {
            realRime.lifecycle.whenReady { realRime.runOnDispatcherWithBoundary(jobId) { block(rimeImpl) } }
        }

        override fun addMessageHandler(handler: (RimeMessage<*>) -> Unit) {
            ensureEstablished { realRime.addMessageHandler(handler) }
        }

        override fun removeMessageHandler(handler: (RimeMessage<*>) -> Unit) {
            ensureEstablished { realRime.removeMessageHandler(handler) }
        }
    ...
    init {
        ...
        TrimeSdk.coroutineScope.launch {
